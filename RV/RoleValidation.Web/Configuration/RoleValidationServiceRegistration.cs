...
public sealed class EmailProcessingCapability
{
    ...
    internal EmailProcessingCapability(
        bool isEnabled,
        IReadOnlyList<string> configurationKeys)
    {
        ConfigurationKeys = Array.AsReadOnly(configurationKeys.ToArray());
        IsEnabled = isEnabled && !HasConfigurationErrors;
    }
    ...
    public bool HasConfigurationErrors => ConfigurationKeys.Count > 0;
}
...
public static class RoleValidationServiceRegistration
{
        ...
        IHostEnvironment environment)
    {
        ...
        try
        {
            options = configuration.GetSection(EmailOptions.SectionName)
                .Get<EmailOptions>() ?? new EmailOptions();
        }
        catch (InvalidOperationException)
        {
            // Binder errors can contain configuration values. Do not expose them.
            return new EmailProcessingCapability(false, [EmailOptions.SectionName]);
        }

        var validator = new EmailOptionsValidator(
            environment.EnvironmentName,
            configuration["RoleValidation:DataSource"]);
        var result = validator.Validate(options, out var configurationKeys);
        return new EmailProcessingCapability(result.Succeeded, configurationKeys);
    }
